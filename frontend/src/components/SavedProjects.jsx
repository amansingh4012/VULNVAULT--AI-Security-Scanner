...
export default function SavedProjects({ onLoadResults }) {
  ...
  const fetchProjects = async (isMounted = { current: true }) => {
    setLoading(true)
    setError(null)
    try {
      ...
      if (isMounted.current) setProjects(response.data.projects)
    } catch (err) {
      console.error('Error fetching projects:', err)
      if (isMounted.current) setError(err.response?.data?.detail || 'Failed to load saved projects. Please ensure MongoDB is running.')
    } finally {
