...
try:
    ...
    print(f"❌ Failed to load projects routes: {e}")
        ...
        if projects_collection is not None:
            try:
                ...
                if user and user.get("user_id"):
                    project_data["user_id"] = user.get("user_id")
                # Use update_one with upsert to handle duplicate project names
                projects_collection.update_one(
                    {"project_name": project_name.strip(), "user_id": project_data.get("user_id")},
                    {"$set": project_data},
                    upsert=True
                )
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            for member in zip_ref.namelist():
                ...
                if not member_path.startswith(os.path.realpath(extract_dir)):
                    raise HTTPException(400, "Malicious ZIP file detected: path traversal attempt")
...
async def scan_github_repo(
    ...
    try:
        ...
        try:
            try:
                ...
                if r.status_code == 200:
                    with zipfile.ZipFile(io.BytesIO(r.content)) as z:
                        # Validate against ZipSlip
                        for member in z.namelist():
                            member_path = os.path.realpath(os.path.join(temp_dir, member))
                            if not member_path.startswith(os.path.realpath(temp_dir)):
                                raise HTTPException(400, "Malicious ZIP detected: path traversal attempt")
                        z.extractall(temp_dir)
                    print(f"Successfully downloaded and extracted repository ZIP")
                else:
                    raise Exception(f"Failed to fetch repository via Zip (HTTP {r.status_code}). Ensure Git is installed or the repository is public.")
            ...
            if projects_collection is not None:
                try:
                    ...
                    if user and user.get("user_id"):
                        project_data["user_id"] = user.get("user_id")
                    # Use upsert to handle duplicate project names gracefully
                    projects_collection.update_one(
                        {"project_name": project_name.strip(), "user_id": project_data.get("user_id")},
                        {"$set": project_data},
                        upsert=True
        ...
        # Check if python git module couldn't be imported or initialized
        if isinstance(e, ImportError) and 'git' in str(e).lower():
            raise HTTPException(500, "Server configuration error: Git is not installed or not in PATH.")
            
        # Check for GitCommandError safely
        if 'git' in locals() and hasattr(git, 'GitCommandError') and isinstance(e, git.GitCommandError):
...
async def scan_project_dependencies(
    ...
    try:
        ...
        if is_zip:
            ...
            # Extract ZIP safely (prevent ZipSlip)
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                for member in zip_ref.namelist():
                    member_path = os.path.realpath(os.path.join(temp_dir, member))
                    if not member_path.startswith(os.path.realpath(temp_dir)):
...
if __name__ == "__main__":
    ...
    print()
    print("╔══════════════════════════════════════════════════════════╗")
    print("║            🛡️  VulnVault Backend Server                 ║")
    ...
    # MongoDB status
    from app.database import get_connection_status
    db_status = get_connection_status()
    if db_status == "connected":
        print("║  🗄️  MongoDB:      ✅ Connected                       ║")
    elif db_status == "mongomock":
        print("║  🗄️  MongoDB:      ⚠️  In-Memory (mongomock)           ║")
    else:
    ...
    if clerk_client is not None:
        print("║  🔐 Clerk Auth:    ✅ Connected                      ║")
    else:
        print("║  🔐 Clerk Auth:    ❌ Not Configured                  ║")
    
    # Email status
    if EMAIL_ENABLED:
        print("║  📧 Email Alerts:  ✅ Enabled                        ║")
    else:
        print("║  📧 Email Alerts:  ⚠️  Disabled                       ║")
